...
{
    Box b;
    b.x1    = d.x1;      // ← 若 util.h 欄位名不同（例如 left/top/right/bottom）
    b.y1    = d.y1;      //   只要改這 6 行即可
    b.x2    = d.x2;
    b.y2    = d.y2;
    b.score = d.score;
    b.cls   = d.class_id;
...
void kf_predict(KalmanState& s, bool zero_h_velocity)
{
    // Lost 的軌跡不要讓框繼續長大 / 縮小
    ...
    const float sd[8] = {
        kStdPos * h, kStdPos * h, 1e-2f, kStdPos * h,
        kStdVel * h, kStdVel * h, 1e-5f, kStdVel * h
    };

    // x = F x
    for (int i = 0; i < 4; ++i) s.mean[i] += s.mean[i + 4];

    // P = F P Fᵀ：先做列 (F P)，再做行 (· Fᵀ)，兩步互不重疊所以可就地運算
    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 8; ++j)
            s.cov[i][j] += s.cov[i + 4][j];
    for (int i = 0; i < 8; ++i)
        for (int j = 0; j < 4; ++j)
            s.cov[i][j] += s.cov[i][j + 4];
...
void STrack::init(const Box& b, int id, int frame)
{
    float t[4], xyah[4];
    box_to_tlwh(b, t);
    tlwh_to_xyah(t, xyah);
...
void STrack::update(const Box& b, int frame)
{
    float t[4], xyah[4];
    box_to_tlwh(b, t);
    tlwh_to_xyah(t, xyah);
...
void STrack::re_activate(const Box& b, int frame)
{
    float t[4], xyah[4];
    box_to_tlwh(b, t);
    tlwh_to_xyah(t, xyah);
...
void STrack::predict()
{
    kf_predict(kf, state != TrackState::Tracked);
}
...
void BYTETracker::reset()
{
    ...
    new_tracks_.clear();
    output_.clear();
    max_time_lost_ = static_cast<int>(p_.frame_rate / 30.f * p_.track_buffer);
...
const std::vector<Track>& BYTETracker::update(const std::vector<Box>& dets)
{
    ...
    // ── 1. Kalman 預測 ────────────────────────────────────────────────────
    for (STrack& t : tracks_) t.predict();
    ...
    for (const auto& m : matches_) {
        ...
        if (t.state == TrackState::Tracked) t.update(d, frame_id_);
        else                                t.re_activate(d, frame_id_);
    }

    // ── 3. 第二階段：沒配到的 Tracked ↔ 低分框（救遮擋 / 模糊的框）───────
    r_tracked_.clear();
    for (int i : u_track_) {
    ...
    for (const auto& m : matches_)
        tracks_[r_tracked_[m.first]].update(low_[m.second], frame_id_);
    ...
    for (const auto& m : matches_)
        tracks_[unconfirmed_[m.first]].update(rest_high_[m.second], frame_id_);
    ...
    for (STrack& t : tracks_)
        if (t.state == TrackState::Lost &&
            frame_id_ - t.frame_id > max_time_lost_)
            t.mark_removed();

    // ── 7. 整理容器（先去重再刪除，避免指標/索引失效）───────────────────
    remove_duplicates();
