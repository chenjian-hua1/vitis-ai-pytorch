 ...
 *   2. 每批 num_ops 是否恆為 1 或 2（固定 2 拍週期成立的前提）
 *   3. 輸出像素與黃金參考模型逐一比對
...
//#endif
    ...
    if (passed != total) {
        printf("\n除錯建議（依優先順序）：\n");
        printf("  0. 若輸出全為 0 -> 檢查 kernel 簽名與 tb 呼叫的參數是否一致\n");
        printf("  1. 若 leftover 序列就錯 -> 檢查 op_bits 與 do_op 的計算\n");
        printf("  2. 若只有某一通道錯     -> 檢查 px0~px3 的 .range() 切片位置\n");
        printf("  3. 若三通道錯誤數相同   -> 檢查 ox / row_in_block 的推進條件\n");
        printf("  4. 若差值很小(<=2)      -> 可能只是定點捨入，可接受\n");
        printf("  5. 若小尺寸過大尺寸錯   -> 檢查 acc/acc_len 的殘餘處理\n");
    }
