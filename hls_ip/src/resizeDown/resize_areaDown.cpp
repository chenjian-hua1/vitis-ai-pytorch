...
#define ACCW       14       /* 9 個 8-bit 樣本最大 2295 */
                         ...
                         ap_uint<16>                inv_scale)
{
    ...
    ap_uint<ACCW> lb0_r[QUAD_W_MAX], lb0_g[QUAD_W_MAX], lb0_b[QUAD_W_MAX];
    ap_uint<ACCW> lb1_r[QUAD_W_MAX], lb1_g[QUAD_W_MAX], lb1_b[QUAD_W_MAX];
    ap_uint<ACCW> lb2_r[QUAD_W_MAX], lb2_g[QUAD_W_MAX], lb2_b[QUAD_W_MAX];
    ap_uint<ACCW> lb3_r[QUAD_W_MAX], lb3_g[QUAD_W_MAX], lb3_b[QUAD_W_MAX];
#pragma HLS BIND_STORAGE variable=lb0_r type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb0_g type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb0_b type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb1_r type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb1_g type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb1_b type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb2_r type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb2_g type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb2_b type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb3_r type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb3_g type=RAM_T2P impl=BRAM
#pragma HLS BIND_STORAGE variable=lb3_b type=RAM_T2P impl=BRAM
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=QUAD_W_MAX
        lb0_r[i] = 0; lb0_g[i] = 0; lb0_b[i] = 0;
        lb1_r[i] = 0; lb1_g[i] = 0; lb1_b[i] = 0;
        lb2_r[i] = 0; lb2_g[i] = 0; lb2_b[i] = 0;
        lb3_r[i] = 0; lb3_g[i] = 0; lb3_b[i] = 0;
    }
...
#pragma HLS DEPENDENCE variable=lb1_r inter false
#pragma HLS DEPENDENCE variable=lb1_g inter false
#pragma HLS DEPENDENCE variable=lb1_b inter false
#pragma HLS DEPENDENCE variable=lb2_r inter false
#pragma HLS DEPENDENCE variable=lb2_g inter false
#pragma HLS DEPENDENCE variable=lb2_b inter false
#pragma HLS DEPENDENCE variable=lb3_r inter false
#pragma HLS DEPENDENCE variable=lb3_g inter false
#pragma HLS DEPENDENCE variable=lb3_b inter false
...
#pragma HLS UNROLL
            ...
            ap_uint<ACCW> c0_r = lb0_r[base_idx], c0_g = lb0_g[base_idx], c0_b = lb0_b[base_idx];
            ap_uint<ACCW> c1_r = lb1_r[base_idx], c1_g = lb1_g[base_idx], c1_b = lb1_b[base_idx];
            ap_uint<ACCW> c2_r = lb2_r[base_idx], c2_g = lb2_g[base_idx], c2_b = lb2_b[base_idx];
            ap_uint<ACCW> c3_r = lb3_r[base_idx], c3_g = lb3_g[base_idx], c3_b = lb3_b[base_idx];
...
#pragma HLS BIND_OP variable=m3_b op=mul impl=dsp
            ...
            if (s3) {
                if (bsel) {
                    lb2_r[base_idx] = w0_r; lb2_g[base_idx] = w0_g; lb2_b[base_idx] = w0_b;
                    lb3_r[base_idx] = w1_r; lb3_g[base_idx] = w1_g; lb3_b[base_idx] = w1_b;
                } else {
                    lb0_r[base_idx] = w0_r; lb0_g[base_idx] = w0_g; lb0_b[base_idx] = w0_b;
                    lb1_r[base_idx] = w1_r; lb1_g[base_idx] = w1_g; lb1_b[base_idx] = w1_b;
                }
            } else {
                lb0_r[base_idx] = w0_r; lb0_g[base_idx] = w0_g; lb0_b[base_idx] = w0_b;
                lb1_r[base_idx] = w1_r; lb1_g[base_idx] = w1_g; lb1_b[base_idx] = w1_b;
                lb2_r[base_idx] = w2_r; lb2_g[base_idx] = w2_g; lb2_b[base_idx] = w2_b;
                lb3_r[base_idx] = w3_r; lb3_g[base_idx] = w3_g; lb3_b[base_idx] = w3_b;
            }
