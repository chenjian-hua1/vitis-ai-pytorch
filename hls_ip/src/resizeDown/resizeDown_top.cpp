...
#pragma HLS BIND_STORAGE variable=lbB type=RAM_S2P impl=BRAM
    ...
    /* 輸入 128-bit 拍數 = 總 byte 數 / 16 */
    const ap_uint<32> total_words = (ap_uint<32>(img_w) * ap_uint<32>(img_h) * 3) >> 4;
...
#pragma HLS BIND_STORAGE variable=word_ch   type=fifo impl=srl
    ...
    compute_side  (in_ptr, result_ch, img_w, img_h, scale_mode);
    pack_side     (result_ch, word_ch, img_w, img_h, scale_mode);
    axi_write_side(word_ch, out_ptr, img_w, img_h, scale_mode);
}
